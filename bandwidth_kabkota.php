...
$menu="4";
 ...
 $title="Top ".$_GET['jar']." Bandwidth Usage Per Kabupaten/Kota";
 $data = "['Firefox',   45.0],
                ['IE',       26.8],
                ['Chrome', 12.8],
                ['Safari',    8.5],
...
if($vjar==""){
	...
	if($vdf=="" || $vdt=="" || ($vdf==$skr && $vdt==$skr)){
		$vdf=$skr;
		$vdt=$vdf;
		$sql="select group3,sum(trafficin) from  daily join hosts on host=hostid where $s_ORG group by group3 order by sum(trafficin) desc limit 0,".$vjar;
	}else{
		$sql="select group3,sum(trafficin) from  daily_h join hosts on host=hostid where $s_ORG and checkdt between '".$vdf." 00:00:00' and '".$vdt." 23:59:59' group by group3 order by sum(trafficin) desc limit 0,".$vjar;
	}
	//echo $sql;
	$res=exec_qry($conn,$sql);
	$i=0;
...
<html lang="en-gb" dir="ltr">
    ...
    <head>
        ...
        <title><?php echo $c_site_title;?> - Bandwidth Per Kabupaten/Kota</title>
...
<div id="container" style="min-width: 310px; height: 350px; margin: 0 auto"></div>
				...
				<tr>
				<th>Kabupaten/Kota</th><th>Total</th>
				</tr>
				<?php
