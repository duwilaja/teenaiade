<?php
include 'inc.const.php';
?>
<!DOCTYPE html>
<html lang="en-gb" dir="ltr" class="uk-height-1-1">
 
    <head>
        <meta charset="utf-8">
        <title><?php echo $c_site_title;?> - Login</title>
        <link rel="shortcut icon" href="images/favicon.ico" type="image/x-icon">
        <link rel="apple-touch-icon-precomposed" href="images/apple-touch-icon.png">
        <link rel="stylesheet" href="uikit/css/uikit.docs.min.css">
        <script src="vendor/jquery.js"></script>
        <script src="uikit/js/uikit.min.js"></script>
    </head>
 
    <body class="uk-height-1-1">
 
        <div class="uk-vertical-align uk-text-center uk-height-1-1">
            <div class="uk-vertical-align-middle" style="width: 250px;">
            <center>
					<!--img src="images/Lambang_TNI_2013.png" width = "90px" height = "90px"-->
					<img src="images/Lambang_TNI_AD.png" width = "90px" height = "90px">
			</center>
			
			<div align="center">
                <!--span class="uk-icon-arrow-circle-o-right uk-icon-large"></span-->
            <h2>Login - TNI AD</h2><small>SISTEM MONITORING JARINGAN TERPADU </small>
			</div>
			
                <!--img class="uk-margin-bottom" width="140" height="120" src="data:image/svg+xml;base64," alt=""-->
 
                <form class="uk-panel uk-panel-box uk-form" method="post" action="login_do.php" name="myf">
                    <div class="uk-form-row">
                        <input class="uk-width-1-1 uk-form-large" name="u" type="text" placeholder="UserID">
                    </div>
                    <div class="uk-form-row">
                        <input class="uk-width-1-1 uk-form-large" name="p" type="password" placeholder="Password">
                    </div>
                    <div class="uk-form-row">
                        <button class="uk-width-1-1 uk-button uk-button-primary uk-button-large">Login</button>
                    </div>
                    <!--div class="uk-form-row uk-text-small">
                        <label class="uk-float-left"><input type="checkbox"> Remember Me</label>
                        <a class="uk-float-right uk-link uk-link-muted" href="#">Forgot Password?</a>
                    </div-->
                </form>
<?php
					if($_GET['m']!=""){ ?>
					<div class="uk-alert uk-alert-danger"><i class="uk-icon-user"></i> <?php echo $_GET['m']; ?></div>
<?php } ?>				
            </div>
        </div>
 
    </body>
 
</html>
