...
$id=$_GET['id'];
 ...
 $conn=connect();
  ...
  $sql="select distinct k.map,h.group3,kr.top,kr.left,count(*) as tot,sum(if((d.ping+d.ping2+log1+log2+log3)>0,1,0)) as akt from kodam k join hosts h on kodamid=group2 join korem kr on kr.korem=group3 and kr.kodam=group2 left join daily d on h.hostid=d.host where $s_ORG and group2='".$id."' group by k.map,h.group3,h.top";
  //echo $sql;
  ...
  //$txt.='<a href="onoff_hosts.php?oo=-1&jar=&grp4='.$row[0].'"><img src="images/'.$color.'_alert.gif" width="15px" height="15px" style="position:absolute;top:'.$row[3].'px;left:'.$row[2].'px;" title="'.$row[1].'&#10;Total:'.$row[4].'&#10;OK:'.$row[5].'&#10;Inactive:'.$inac.'"></a>';
	...
	$coords=($left).','.($top);
	//$coords=($i*10).','.($i*20);
	$txt.='<area id="titik'.$i.'" shape="circle" coords="'.$coords.',8" href="onoff_hosts.php?oo=-1&grp3='.$row[1].'" title="'.$titel.'" alt="'.$titel.'" data-maphilight=\'{"strokeColor":"000000","strokeWidth":3,"fillColor":"'.$color2.'","fillOpacity":0.6,"alwaysOn":true}\'>';
  }
